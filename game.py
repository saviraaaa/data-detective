...
import statistics, json, base64, io, random
...
# --------------------------
# Game helpers (acak, independen dari data impor)
# --------------------------
def generate_game_data():
    """Buat dataset acak antara 6–10 angka integer 10..100"""
    n = random.randint(6, 10)
    return [random.randint(10, 100) for _ in range(n)]

def generate_question(data):
    """Pilih jenis soal acak: mean, median, mode. Kembalikan (qtype, answer, label)."""
    qtype = random.choice(["mean", "median", "mode"])
    if qtype == "mean":
        answer = calc_mean(data)
        label = "Tebak nilai Mean"
    elif qtype == "median":
        answer = calc_median(data)
        label = "Tebak nilai Median"
    elif qtype == "mode":
        answer = calc_mode(data)  # list (bisa lebih atau None)
        if answer is None:
            answer = []
        label = "Tebak Nilai Modus"
    return qtype, answer, label
...
# Updated GAMES_HTML: show question label, data (from game), tombol soal baru
GAMES_HTML = '''
{% extends "base.html" %}
{% block content %}
<div class="card">
  ...
  {% if data %}
    ...
    <form method="POST" action="{{ url_for('check_guess') }}">
      <input type="number" name="guess" step="any" placeholder="Masukkan jawaban..." required>
      <button type="submit">Cek Jawaban</button>
    </form>
    <div style="margin-top:8px;">
      <a href="{{ url_for('games') }}"><button>Soal Baru</button></a>
    </div>
  {% else %}
    <small>Terjadi kesalahan: soal tidak tersedia.</small>
  {% endif %}
</div>
{% endblock %}
'''

# RESULT_HTML updated: menampilkan tipe soal dan jawaban yang benar (format rapi)
RESULT_HTML = '''
{% extends "base.html" %}
{% block content %}
<div class="card">
  ...
  <p>Data: {{ data }}</p>
  <p>Jawabanmu: {{ guess }}</p>
  <p>Jawaban benar: {{ correct_answer }}</p>
  <h3>{{ message }}</h3>
...
# --------------------------
# GAME: generate question tiap buka /games (independen dari data impor)
# --------------------------
@app.route('/games')
def games():
    game_data = generate_game_data()
    qtype, answer, label = generate_question(game_data)

    # simpan ke session agar /check bisa mengakses
    session['game_data'] = game_data
    session['game_qtype'] = qtype
    # simpan answer serializable (list atau number)
    session['game_answer'] = answer

    return render_template_string(GAMES_HTML, data=game_data, label=label)

@app.route('/check', methods=['POST'])
def check_guess():
    # Ambil tebakan user
    guess_raw = request.form.get('guess', type=str)
    try:
        guess = float(guess_raw)
    except Exception:
        guess = None

    data = session.get('game_data', [])
    qtype = session.get('game_qtype')
    answer = session.get('game_answer')

    if qtype is None or answer is None:
        return render_template_string(RESULT_HTML,
                                      label="(soal tidak ditemukan)",
                                      data=data,
                                      guess=guess_raw,
                                      correct_answer="(tidak tersedia)",
                                      message="Soal tidak ditemukan. Silakan mulai soal baru.")

    # cek kebenaran berdasarkan jenis soal
    correct = False
    tol = 0.01  # toleransi untuk perbandingan float

    if qtype in ("mean", "median"):
        # answer harus berupa angka
        try:
            correct = (guess is not None) and (abs(guess - float(answer)) <= tol)
        except Exception:
            correct = False
        correct_answer_display = round(float(answer), 3) if answer is not None else "—"
    else:  # mode
        # answer adalah list (bisa lebih dari 1); terima jika tebakan cocok salah satu (toleransi)
        try:
            if isinstance(answer, list):
                for v in answer:
                    if guess is not None and abs(guess - float(v)) <= tol:
                        correct = True
                        break
            else:
                # jika somehow single value
                correct = (guess is not None) and (abs(guess - float(answer)) <= tol)
        except Exception:
            correct = False
        correct_answer_display = answer if answer is not None else "—"

    message = "Benar banget! 🎉" if correct else "Belum tepat, coba lagi!"
    # format jawaban benar untuk ditampilkan
    if qtype == "mode":
        ca = correct_answer_display
    else:
        ca = correct_answer_display

    return render_template_string(RESULT_HTML,
                                  label=("Tebak " + qtype.capitalize()),
                                  data=data,
                                  guess=guess_raw,
                                  correct_answer=ca,
                                  message=message)
